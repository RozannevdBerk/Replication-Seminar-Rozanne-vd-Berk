...
This codebase is a copy+extension of the codebase created by Swevels et al. for their work (https://github.com/Ava-S/ekg_inference) Contents of their README can be found below.
...
- Run `main.py NoiseCheck` followed by a `1`,`2`,`3`, or `4` (no whitespace) to evaluate the case ID inference method on data with approximately 1%, 5%, 10% and 20% of process steps missing, respectively. The noise dataset were generated by `create_missing_values.py`

`result.txt` adds a line every time the code is run (with the exception of when it is run as just `main.py` without a further argument). For `Short`, it adds a line containing the time it took to run.
